...
pid=$(ps -ax | grep -v grep | grep "^.* ${client_name}.*corbaloc.*3200.*${serverName}" | awk '{print $1}')
[[ ! -z $pid ]] && kill -s 15 $pid && wait 5

# This will connect directly to EchoServer[1,2] via the server endpoint
corbaloc="corbaloc::${host}:32000/${serverName}/EchoServer-POA/${serverName}-ID"
if [[ $serverName == "EchoServer2" ]] ; then
    corbaloc="corbaloc::${host}:32999/${serverName}/EchoServer-POA/${serverName}-ID"
fi
...
cat ${log}
echo "ERROR::$bn: $client_name (${server_name}) may not be running!  Please check the log file"
exit 1
