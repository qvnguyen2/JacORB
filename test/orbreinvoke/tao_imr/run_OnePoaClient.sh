...
if [[ -z $serverName ]] ; then
    echo "Usage: $bn <implName of EchoServer1 or EchoServer2> [POA basename (default=implName)] [first ImR host (default=localhost)] [second ImR host]"
    exit 1
fi
...
export CLASSPATH=${CLASSPATH}:${JACORB_HOME}/classes:`pwd`/build/classes
...
if [[ ! -z $imr_host && ! -z $remote_imr_host ]] ; then
    corbaloc="corbaloc::${imr_host}:44555,:${remote_host}:44555//${serverName}/${poaBaseName}-Parent-POA/${poaBaseName}-POA/${poaBaseName}-ID"
else
    corbaloc="corbaloc::${imr_host}:44555/${serverName}/${poaBaseName}-Parent-POA/${poaBaseName}-POA/${poaBaseName}-ID"
fi
