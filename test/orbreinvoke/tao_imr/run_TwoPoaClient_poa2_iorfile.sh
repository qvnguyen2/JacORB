...
iorfile="/tmp/test.orbreinvoke.tao_imr.TwoPoaServer.${serverName}.${poaBaseName}.ior.child2"
if [[ ! -f $iorfile || ! -r $iorfile ]] ; then
    echo "ERROR::$bn: IOR file $iorfile does not exist or in inaccessible"
    exit 1
fi
echo "$bn: IOR file is ${iorfile}"
echo "$bn: starting ${client_name} ${serverName} ..."
...
if [[ ! -z $pid ]] ; then
	...
	echo "."
        ...
        if ps -p $pid ; then
            tail -5 ${log}
            echo "SUCCESS::$bn: ${client_name} ${serverName} is running."
            exit 0
        fi
        (( cnt = cnt - 1 ))
    done
...
cat ${log}
echo "WARNING::$bn: ${client_name} ${serverName} may not be running!  Please check log file."
exit 1
