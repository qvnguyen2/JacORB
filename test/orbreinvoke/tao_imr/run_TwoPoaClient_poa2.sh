...
if [[ ! -z $imr_host && ! -z $remote_imr_host ]] ; then
    corbaloc="corbaloc::${imr_host}:44555,:${remote_imr_host}:44555/${serverName}/${poaBaseName}-Parent-POA/${poaBaseName}-POA2/${poaBaseName}-ID2"
else
    corbaloc="corbaloc::${imr_host}:44555/${serverName}/${poaBaseName}-Parent-POA/${poaBaseName}-POA2/${poaBaseName}-ID2"
fi
echo "$bn: IOR is ${corbaloc}"
